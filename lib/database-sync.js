...
module.exports = class SQLiteDatabaseSync {
  static staticExtensions() {
    try {
      return binding.staticExtensions()
    } catch (err) {
  ...
  loadStaticExtension(name) {
    if (this._handle === null) {
      throw errors.DATABASE_NOT_OPEN('Database is not open')
    }

    if (typeof name !== 'string' || name.length === 0) {
      throw errors.INVALID_ARGUMENT('Static extension name must be a non-empty string')
    }

    try {
      binding.loadStaticExtension(this._handle, name)
    } catch (err) {
      throw errors.from(err)
    }
  }
