...
module.exports = class SQLiteStatementSync {
  ...
  all(...params) {
    ...
    try {
      return wrapObjectRows(binding.all(this._handle, named, positional))
    } catch (err) {
      throw errors.from(err)
    }
  }
  ...
  values(...params) {
    ...
    try {
      return wrapValueRows(binding.values(this._handle, named, positional))
    } catch (err) {
      throw errors.from(err)
    }
  }
